...
const CurrentEvents = () => {
  ...
  const getAllCurrentEventsList = async () => {
    setLoading(true);
    try {
      ...
      toast(
        <p className="font-bold text-sm text-red-500">
          Internal error ocured while fetching current events
        </p>
      );
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    getAllCurrentEventsList();
  }, []);

  return (
    <div className="flex flex-col items-center justify-center">
      <h2 className="font-semibold text-2xl">Current Events</h2>
      {/* current event list */}
      <div className="my-10">
        {!loading ? (
