...
import { getAllExpiredEvents } from "@/services/EventService";
import moment from "moment";
import React, { useEffect, useState } from "react";
import { toast } from "sonner";

const ExpiredEvents = () => {
  const [expiredEvents, setExpiredEvents] = useState<[] | any>([]);
  const [loading, setLoading] = useState<boolean>(false);

  const getAllExpiredEventsList = async () => {
    setLoading(true);
    try {
      const result = await getAllExpiredEvents(moment().format("MM-DD-YYYY"));
      if (result) {
        setExpiredEvents(result?.data);
      }
    } catch (error) {
      toast(
        <p className="font-bold text-sm text-red-500">
          Internal error ocured while fetching past events
        </p>
      );
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    getAllExpiredEventsList();
  }, []);

  return (
    <div className="flex flex-col items-center justify-center">
      <h2 className="font-semibold text-2xl">Past Events</h2>
      {/* current event list */}
      <div className="my-10">
        {!loading ? (
